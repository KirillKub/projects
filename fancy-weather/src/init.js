...
const URLS = {
  IP: 'https://ipinfo.io/json?token=',
  WEATHER: 'https://api.weatherbit.io/v2.0',
  UNSPLASH: 'https://api.unsplash.com/photos/random?query=',
  PHOTO: 'https://raw.githubusercontent.com/KirillKub/ImageHelp/master/images/weather/',
  YANDEX: 'https://translate.yandex.net/api/v1.5/tr.json/translate?lang=',
};
...
const location = makeRequest(`${URLS.IP}${process.env.KEY_IP}`);
...
async function weatherOnWeek() {
  ...
  date = new Date(date);
  document.getElementsByClassName('main__section3__weather-day')[0].innerHTML = `${ENG.DAY_FULL[(+date.getDay() + 1) % countDay]}`;
  document.getElementsByClassName('main__section3__weather-day')[1].innerHTML = `${ENG.DAY_FULL[(+date.getDay() + 2) % countDay]}`;
  document.getElementsByClassName('main__section3__weather-day')[2].innerHTML = `${ENG.DAY_FULL[(+date.getDay() + 3) % countDay]}`;
}
...
async function createTemperature() {
  ...
  const req = makeRequest(`${URLS.WEATHER}/current?city=${town},${countryName}&key=${process.env.KEY_WEATHERBIT}`);
  try {
...
async function createWeather() {
  ...
  const req = makeRequest(`${URLS.WEATHER}/forecast/daily?city=${town},${countryName}&key=${process.env.KEY_WEATHERBIT}&days=4`);
  try {
    const weatherNow = await req;
    for (let i = 0; i < 3; i += 1) {
      const img = document.createElement('img');
      ...
      const weather = weatherNow.data[i + 1];
      span.innerHTML = `${Math.round(weather.temp)}°`;
      img.src = `${URLS.PHOTO}${weather.weather.icon}.png`;
...
function create(data, className) {
...
async function getBackgroundImg(lang) {
  ...
  if (lang === 'BY') {
    ...
    try {
      const req = makeRequest(`${URLS.UNSPLASH}${weather}+${time}+${SEASON[date.getMonth()]}&client_id=${process.env.KEY_UNSPLASH}`);
  ...
  const reqWeather = makeRequest(`${URLS.WEATHER}/current?city=${town}&key=${process.env.KEY_WEATHERBIT}`);
  try {
...
function createPage() {
  ...
  timeNow();
  weatherOnWeek();
  setTimeout(getBackgroundImg, 1000);
}

export {
  createPage, getBackgroundImg, getTown, timeNow, ENG, addMap, URLS,
};
