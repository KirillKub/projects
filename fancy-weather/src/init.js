...
async function createTemperature() {
  ...
  const req = requestItem(`https://api.weatherbit.io/v2.0/current?city=${town},${countryName}&key=1231c4541af04c7ea7f7b23e65949802`);
  try {
...
async function createWeather() {
  ...
  const req = requestItem(`https://api.weatherbit.io/v2.0/forecast/daily?city=${town},${countryName}&key=1231c4541af04c7ea7f7b23e65949802&days=4`);
  for (let i = 0; i < 3; i += 1) {
    const img = document.createElement('img');
    const span = document.createElement('span');
...
async function backgroundImg() {
  ...
  const reqWeather = requestItem(`https://api.weatherbit.io/v2.0/current?city=${town}&key=1231c4541af04c7ea7f7b23e65949802`);
  try {
  ...
  try {
    const req = requestItem(`https://api.unsplash.com/photos/random?query=${weather},${time},${SEASON[date.getMonth()]}&client_id=dda88c275d8df29539976c1d2daef60cbafb230b82e8e71793e4af7e4b11a79b`);
