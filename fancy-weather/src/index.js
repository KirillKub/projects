...
async function time() {
  ...
  const reqWeather = requestItem(`https://api.weatherbit.io/v2.0/current?city=${TOWN},${country}&key=1231c4541af04c7ea7f7b23e65949802`);
  try {
...
document.getElementsByClassName('main__section1__switch-temperature')[0].addEventListener('click', async function swapTemperature(event) {
  ...
  const reqWeather = requestItem(`https://api.weatherbit.io/v2.0/current?city=${town}&key=1231c4541af04c7ea7f7b23e65949802&${temp}`);
  try {
    const weatherNow = await reqWeather;
    const data = weatherNow.data[0];
  ...
  const req = requestItem(`https://api.weatherbit.io/v2.0/forecast/daily?city=${town}&key=1231c4541af04c7ea7f7b23e65949802&${temp}&days=4`);
  for (let i = 0; i < 3; i += 1) {
    try {
...
async function search() {
  ...
  const reqWeather = requestItem(`https://api.weatherbit.io/v2.0/current?city=${town}&key=1231c4541af04c7ea7f7b23e65949802&${temp}&lang=${lang}`);
  try {
...
document.getElementsByClassName('main__section1__switch-lang')[0].addEventListener('click', async function changeLang(event) {
  ...
  if (event.target.classList.contains('lang-EN') || event.target.classList.contains('lang-RU') || event.target.classList.contains('lang-BY')) {
    const reqWeather = requestItem(`https://api.weatherbit.io/v2.0/current?city=${TOWN}&key=1231c4541af04c7ea7f7b23e65949802&lang=${langNow}`);
    try {
      const weatherNow = await reqWeather;
      const data = weatherNow.data[0];
